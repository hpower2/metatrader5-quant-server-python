...
2. **Configure Environment Variables**
   ...
   The stack ships with safe local defaults, but you should still create a `.env` file before production use:
   ...
   TRAEFIK_HASHED_PASSWORD=your-apr1-hash
   ACME_EMAIL=youremail@example.com
   ```

   **Note:** To generate the Traefik HTTP Basic Auth hash, you can use:
...
For local development, the stack also exposes direct service ports, so you can reach Django at `http://127.0.0.1:8000` and Grafana at `http://127.0.0.1:3000` without configuring DNS first.
...
Local defaults use `*.localhost` hostnames so the stack can start without a custom `.env`, but Let's Encrypt only works with real public domains. Set real domain values in `.env` before relying on Traefik HTTPS certificates.
